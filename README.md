...
A Firmware based upon JELOS and has a similar feature set to PlumOS/AmberELEC, supporting both Single Card and 2 Card Setups. Has the USB Network Gadget and File Transfer Gadget(MTP) settings to transfer files via USB Cable. A stable Firmware option if you want the visual appeal of Emulation Station and the ability to use a Wifi/Ethernet USB Dongle for the Scraper or Retroachievements. Based on testing the latest May, 2025 release of ROCKNIX, the Dreamcast performance is far worse compared to PlumOS and I would consider it unplayable. Also, the controller configuration for MupenN64 Standalone is incorrect and PPSSPP needs analog stick settings modified, but these are minor issues(Fixes are listed below in the RockNIX Known Issues section). 
